...
class GenericCollection<T extends IHasTimestamp & Comparable<T>> {
    private HashMap<String, TreeSet<T>> elements;
    ...
    public void addGenericItem(String category, T element) {
        elements.computeIfAbsent(category, key -> new TreeSet<>()).add(element);
    }
    ...
    public Map<String, Set<T>> byMonthAndDay() {
        return elements.values().stream()
                ...
                .collect(Collectors.groupingBy(
                        t -> customDatePrint(t.getTimestamp()),
                        TreeMap::new,
                        Collectors.toCollection(() -> new TreeSet<T>(Comparator.reverseOrder()))));
    }

    public static String customDatePrint(LocalDateTime date) {
    ...
    public HashMap<String, TreeSet<T>> getElements() {
        return elements;
    }
}
