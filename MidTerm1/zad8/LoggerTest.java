    ...
    void printResults() {
        ...
        LogProcessor<T> firstLogProcessor = new LogProcessor<T>() {
            ...
            public ArrayList<T> processLogs(ArrayList<T> logs) {
                return logs.stream().filter(log -> log.getType().equals("INFO")).collect(Collectors.toCollection(ArrayList::new));
            }
        };
        ...
        LogProcessor<T> secondLogProcessor = new LogProcessor<T>() {
            @Override
            public ArrayList<T> processLogs(ArrayList<T> logs) {
                return logs.stream().filter(log -> log.getMessage().length() < 100).collect(Collectors.toCollection(ArrayList::new));
            }
        };
        ...
        LogProcessor<T> thirdLogProcessor = new LogProcessor<T>() {
            @Override
            public ArrayList<T> processLogs(ArrayList<T> logs) {
                return logs.stream().sorted(Comparator.comparing(ILog::getTimestamp)).collect(Collectors.toCollection(ArrayList::new));
            }
        };
